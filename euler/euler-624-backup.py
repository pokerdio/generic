...
def mm(m1, m2):
    return [vv(m1[0:2], m2[0::2]), vv(m1[0:2], m2[1::2]),
            vv(m1[2:], m2[0::2]), vv(m1[2:], m2[1::2])]


def mv(m, v):
    return [vv(m[0:2], v), vv(m[2:], v)]
...
def mpow(m, k):
    ...
    mpow = [m]
    while two <= k:
        print("first run ", two, k)
        mpow.append(mm(mpow[-1], mpow[-1]))
        two *= 2
    ret = (1, 0, 0, 1)
    i, two = 0, 1
    while two <= k:
        print("second run", two, k)
        if two & k:
            ret = mm(ret, mpow[i])
        i += 1
        two *= 2
...
def minv(m):
    det = m[0] * m[3] - m[1] * m[2]
    return [F(m[3], det), -F(m[1], det), -F(m[2], det), F(m[0], det)]


def go(n, p):
    m = (0, F(1, 2), F(1, 2), F(1, 2))
    v0 = (F(1, 4), ) * 2
    ...
    mn = mm(m0, m3)
    print("b")
    mtot = mm(minv([1-mn[0], -mn[1], -mn[2], 1-mn[3]]), m0)
    print("c")
    vret = mv(mtot, v0)

    print("d")
    return Q(F(vret[1], 2), p)
