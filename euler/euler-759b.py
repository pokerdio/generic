...
_sgg0, _sgg1, _sgg2, _sgg3, _sgg4 = (_sg_maker(x, 2) for x in range(5))
...
def sgg3(segment, delta=0):
    if segment < 3:
        return _sgg3(segment, delta)
    up, base, delta = fix_interval(segment, delta)

    evens = sgg3(segment - 1, (delta + 1) // 2) * 8

#    sum((g2(n) +2g(n)+ 1) * (8n3+12n2+6n+1))

    odds = sgg0(segment - 1, (delta // 2)) + \
        sgg1(segment - 1, (delta // 2)) * 6 + \
        sgg2(segment - 1, (delta // 2)) * 12 + \
        sgg3(segment - 1, (delta // 2)) * 8 + \
        sg0(segment - 1, (delta // 2)) * 2 + \
        sg1(segment - 1, (delta // 2)) * 12 + \
        sg2(segment - 1, (delta // 2)) * 24 + \
        sg3(segment - 1, (delta // 2)) * 16 + \
        sum_odds_cubed_interval(base + 1, delta // 2)
    return evens + odds
...
def sgg4(segment, delta=0):
    if segment < 3:
        return _sgg4(segment, delta)
    up, base, delta = fix_interval(segment, delta)

    evens = sgg4(segment - 1, (delta + 1) // 2) * 16

#    sum((g2(n)+2g(n) + 1) * (16n4+ 32n3+24n2+8n+1))

    odds = sgg0(segment - 1, (delta // 2)) + \
        sgg1(segment - 1, (delta // 2)) * 8 + \
        sgg2(segment - 1, (delta // 2)) * 24 + \
        sgg3(segment - 1, (delta // 2)) * 32 + \
        sgg4(segment - 1, (delta // 2)) * 16 + \
        sg0(segment - 1, (delta // 2)) * 2 + \
        sg1(segment - 1, (delta // 2)) * 16 + \
        sg2(segment - 1, (delta // 2)) * 48 + \
        sg3(segment - 1, (delta // 2)) * 64 + \
        sg4(segment - 1, (delta // 2)) * 32 + \
        sum_odds_quadded_interval(base + 1, delta // 2)
    return evens + odds
