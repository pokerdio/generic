class PixelSprite {
	...
	let ret = new PixelSprite(w, h);
	for (let i=0 ; i<h ; ++i) {
	    for (let j=0 ; j<w ; ++j) {
		ret.setPixel(j, i, this.getPixel(x + j, y + i))
	    }
	}
	return ret; 
    }
	...
	for (let i=0 ; i<this.rows ; ++i) {
	    for (let j=0 ; j<x ; ++j) {
		this.setPixel(j, i, this.getPixel(x * 2 - j - 1, i));
	    }
	}
    }
    mirrorRight(x) {
	for (let i=0 ; i<this.rows ; ++i) {
	    for (let j=x+1 ; j<this.cols ; ++j) {
		this.setPixel(j, i, this.getPixel(x * 2 - j + 1, i));
	    }
	}
    }
